...
def create_py(yamlDir="", whichYaml=None, pyPath=""):
    # 读取CASE_TEMPLATE文件
    with open(filepath.CASE_TEMPLATE, encoding='utf-8') as fr:
        src_content = fr.read()
    dire = yamlDir.split("\\")
    if len(yamlDir.split("\\")) > 1:
        yamlDir, secondDir = yamlDir.split("\\")
        yamlPath = os.path.join(filepath.DATA_PATH, yamlDir, secondDir)
    else:
        yamlPath = os.path.join(filepath.DATA_PATH, yamlDir)
    ...
    for file in all_file:
        ...
        casePath = os.path.join(filepath.CASE_PATH, pyPath)
...
if __name__ == "__main__":
    yamlPath = "4工单售后运维"  # yamlCase下面模块路径，为空表示选择的yaml文件yamlCase根目录下*.yaml文件
    whichYaml = 10 # None yamlPath 目录下所有的yaml文件都生成py,1表示目录下第一个文件
    pyPath = "c:\\tanwinjob\\interface test\\apiautotest-tanwin\\cases\\4工单售后运维"        # case下面模块路径,为空表示生成py文件放在case根目录
