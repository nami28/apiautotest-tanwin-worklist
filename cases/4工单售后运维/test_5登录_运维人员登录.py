# coding=utf-8 

from ddt import ddt, data
import unittest
import json
from common.check import Check
from common.readConfig import confParam
from common.readToken import write_file
from common.findReplace import findAndReplace
from common.readYaml import readYaml
from getRootPath import root_dir
from common.writeLog import writeLog
import os
from common.sendRequest import sendRequest
#from common.do_regex import DoRegex


@ddt
class test_5运维人员登录(unittest.TestCase):
    yamlPath = ['4工单售后运维', '3登录_运维人员登录.yaml']
    yaml_path = os.path.join(root_dir, "yamlCase")
    for dir in yamlPath:
        yaml_path = os.path.join(yaml_path, dir)
    read_yaml = readYaml(yaml_path)
    case_list = read_yaml.caseList()
    #print(case_list)
    case_list = eval(findAndReplace(str(case_list), {"workstaff": confParam("workstaff")}))
    #case_list = eval(DoRegex.do_regex(str(case_list)))
    #print(case_list)

    method = case_list[0]["method"]
    url = case_list[1]["url"]

    # 跳过说明
    reason = confParam("skip_reason")

    @classmethod
    def setUpClass(cls):

        # 声明dict变量存储token
        cls.token_dict = {}

        # 拼接接口地址
        cls.url = confParam("hostName") + cls.url
        cls.client = sendRequest()

        # 请求信息头
        cls.headers = {"Content-Type": "application/x-www-form-urlencoded;charset=UTF-8"}

    # case_list传进去做数据驱动
    @data(*case_list[2:])
    def test_5运维人员登录(self, cases):

        for caseName, caseInfo in cases.items():
            caseName = caseName
            data = caseInfo["data"]
            check = caseInfo["check"]
            self.__dict__['_testMethodDoc'] = caseName

        # 发送请求
        response = self.client.sendRequest(self.method, self.url, self.headers, data)

        # 接口返回文本信息
        text = response.text

        # 把文本信息转化为字典格式
        text_dict = json.loads(text)

        # 判断接口响应code是否为200，获取token
        if text_dict["code"] == 0:
            self.token_dict["token"] = response.headers['Set-Cookie'].split(";")[0].split("=")[1]

        # 写日志
        writeLog(caseName, self.url, data, check, text_dict)

        # 断言
        Check().check(check, text_dict)

    @classmethod
    def tearDownClass(cls):

        # 把token写入文件
        write_file(json.dumps(cls.token_dict))


if __name__ == "__main__":
    unittest.main()
