...
@ddt
class test_新增工单获取项目详情(unittest.TestCase):
    yamlPath = ['4工单售后运维', '新增工单获取项目详情.yaml']
    ...
    projectId = db.getprojectId("project_id", "work_order_info")
    case_list = eval(findAndReplace(str(case_list), {"projectId": projectId}))
    ...
    # case_list传进去做数据驱动
    @data(*case_list[2:])
    def test_新增工单获取项目详情(self, cases):
